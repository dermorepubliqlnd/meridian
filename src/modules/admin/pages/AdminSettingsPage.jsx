...
function useAdminList(docId, defaults) {
  ...
  const load = async () => {
    ...
    else { await setDoc(ref, { items: defaults }); setItems(defaults); }
  };
...
function SettingsList({ docId, defaults, label, description }) {
  const { items, addItem, removeItem } = useAdminList(docId, defaults);
...
function WorkCalendarSection() {
  ...
  useEffect(() => {
    getDoc(ref).then((snap) => {
      if (snap.exists()) setCalendar(snap.data());
      else setDoc(ref, CALENDAR_DEFAULT).then(() => setCalendar(CALENDAR_DEFAULT));
    });
  }, []);

  if (!calendar) return <p className="text-[12px] text-gray-400">Loading...</p>;
  ...
  const toggleWorkDay = async (day) => {
    ...
    const next = { ...calendar, workDays: updated };
    setCalendar(next);
  ...
  const setCapacityHours = async (val) => {
    ...
    const next = { ...calendar, dailyCapacityHours: hours };
    setCalendar(next);
    setSaving(true);
    await setDoc(ref, next);
    setSaving(false);
  ...
  const addHoliday = async () => {
    ...
    const next = { ...calendar, holidays: [...calendar.holidays, { date, name: name.trim() }].sort((a, b) => a.date.localeCompare(b.date)) };
    setCalendar(next);
    await setDoc(ref, next);
  ...
  const removeHoliday = async (date) => {
    const next = { ...calendar, holidays: calendar.holidays.filter((h) => h.date !== date) };
    setCalendar(next);
    await setDoc(ref, next);
  };

  const weeklyCapacity = calendar.dailyCapacityHours * calendar.workDays.length;
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-4 col-span-2">
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="flex items-center gap-3">
              <input
                ...
                value={calendar.dailyCapacityHours}
              ...
              />
              <span className="text-[12px] text-gray-500">hrs/day {saving && <span className="text-gray-400">saving…</span>}</span>
            </div>
          </div>
            ...
            <div className="flex gap-2">
              {DAY_LABELS.map((d, i) => (
                <button
                  ...
                  className={`w-9 h-9 rounded-full text-[11px] font-medium transition ${
                    calendar.workDays.includes(i)
                      ? "bg-navy text-white"
          ...
          <div className="bg-teal/10 rounded-md p-3 text-[12px] text-teal-800">
            ...
            <div className="text-[11px] text-teal-600 mt-0.5">
              ...
              &nbsp;·&nbsp; 4-week reference capacity: <strong>{weeklyCapacity * 4} hrs</strong>
            </div>
          </div>
        </div>
        ...
        <div>
          <label className="text-[11px] text-gray-500 uppercase tracking-wide font-medium mb-2 block">
            Company Holidays <span className="font-normal normal-case text-gray-400">({calendar.holidays.length})</span>
          </label>
          <div className="max-h-52 overflow-y-auto divide-y divide-gray-50 mb-3 border border-gray-100 rounded-md">
            {calendar.holidays.map((h) => (
              <div key={h.date} className="flex items-center justify-between px-2.5 py-1.5 hover:bg-slate-50 group">
                ...
                <button onClick={() => removeHoliday(h.date)} className="text-[11px] text-gray-300 hover:text-red-400 opacity-0 group-hover:opacity-100 transition">
                  ✕
                </button>
              </div>
            ))}
            {calendar.holidays.length === 0 && (
              <p className="px-3 py-3 text-[12px] text-gray-400 italic">No holidays added.</p>
            )}
          </div>
          <div className="flex gap-2">
            <input
            ...
            />
            <button onClick={addHoliday} className="text-[12px] bg-navy text-white px-3 py-1.5 rounded-md hover:bg-navy-light">
              + Add
            </button>
          </div>
        </div>
